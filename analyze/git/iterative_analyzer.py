...
class IterativeGitrepoAnalyzer(GitRepoAnalyzer):
    def __init__(self, repository_path, time_delta):
        super().__init__(repository_path, time_delta)
        self.start_date = (datetime.today() - timedelta(days=time_delta)).strftime('%Y-%m-%d %H:%M:%S %z')
